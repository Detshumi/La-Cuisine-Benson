<##
migrate-prod-to-dev-options.ps1

Usage:
    .\scripts\migrate-prod-to-dev-options.ps1 [-Table options] [-UseSsh] [-SshUser root] [-SshPort 22] [-NoBackup] [-KeepDump] [-Force]

What it does:
 - Reads DB credentials for prod and dev from the project's `.env` file (DB_*_prod and DB_*_dev or fallbacks).
 - Dumps only the specified table from the production DB (default `options`).
 - Optionally runs pg_dump on the remote server over SSH and streams the dump locally.
 - Imports the single-table dump into the dev database (will drop the table if --clean is present in the dump).
 - Optionally saves a backup of the dev table before restoring.

Warnings:
 - This is a destructive operation for the specified table in the development DB (it will be replaced by a copy of prod).
...
try {
    ...
    Write-Host "PROD -> DEV single-table migration plan"
    Write-Host ("  Table: {0}" -f $Table)
    Write-Host ("  Prod: {0}@{1}:{2}/{3}" -f $prodUser, $prodHost, $prodPort, $prodDb)
    Write-Host ("  Dev:  {0}@{1}:{2}/{3}" -f $devUser, $devHost, $devPort, $devDb)

    if (-not $Force) {
        $confirm = Read-Host "Proceed? This will overwrite the development table '$Table'. Type 'yes' to continue"
    ...
    $dumpFile = Join-Path $env:TEMP "prod_${Table}_dump_$timestamp.sql"

    # Backup dev table by default unless -NoBackup is specified
    if (-not $NoBackup) {
        $devBackupFile = Join-Path $env:TEMP "dev_${Table}_backup_$timestamp.sql"
        Write-Host "Backing up dev table '$Table' to $devBackupFile"
        $env:PGPASSWORD = $devPass
        & pg_dump -h $devHost -p $devPort -U $devUser -F p -t $Table -f $devBackupFile $devDb
        if ($LASTEXITCODE -ne 0) { throw "Failed to backup dev table (pg_dump exit code $LASTEXITCODE)" }
    ...
    if ($UseSsh) {
        ...
        $remoteCmd = "PGPASSWORD='$prodPass' pg_dump --format=plain --clean --if-exists --no-owner --no-acl -U $prodUser -h $prodHost -p $prodPort -t $Table $prodDb"
        ...
        & pg_dump --format=plain --clean --if-exists --no-owner --no-acl -h $prodHost -p $prodPort -U $prodUser -t $Table -f $dumpFile $prodDb
        if ($LASTEXITCODE -ne 0) { throw "pg_dump failed (exit code $LASTEXITCODE)." }
        Remove-Variable -Name PGPASSWORD -ErrorAction SilentlyContinue
    }

    # Restore into dev DB
    Write-Host "Restoring table dump into development DB..."
    $env:PGPASSWORD = $devPass
    ...
    Write-Host "Single-table migration completed successfully."
